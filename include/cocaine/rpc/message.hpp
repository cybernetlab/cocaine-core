...
/**
 * @brief Implements cocaine RPC protocol message
 * @details All communications between components are maded via messages.
 *          Every message is a MessagePack-ed tuple of three fields:
 *          `channel_id`, `message_id` and a tuple of slot-specific stuff.
 *          `channel_id` is a way to multiplex multiple dataflows inside a
 *          single TCP session. `channel_id` generated by caller. `message_id`
 *          is an unique service slot number and can be obtained via
 *          locator_t::resolve.
 */
struct message_t {
    ...
    /**
     * @brief Casts message body (slot specific args) to specified list of
     *        arguments.
     *
     * @param targets list of arguments acceptors
...
public:
    ...
    /**
     * @brief Returns message body as msgpack object
